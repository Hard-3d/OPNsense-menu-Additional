...
```sh
cd /
unzip -o /root/opnsense-additional-menu-v0.1.38-root.zip
...
- tag: `v0.1.38`
- asset: `opnsense-additional-menu-v0.1.38-root.zip`

В самой странице **Update** укажите:

```text
Repository URL: https://github.com/OWNER/REPO
Release asset name: opnsense-additional-menu-v0.1.38-root.zip
```
...
2. Создайте новый git tag, например `v0.1.38`.
...
- Rollback of v0.1.38: removed automatic udp2raw restart before WireGuard restart in Status WireGuard.
- WireGuard check behavior returned to v0.1.38 logic: on degradation it restarts WireGuard only.
...
- udp2raw: исправлена JS-ошибка формы из v0.1.38, из-за которой не загружались статус бинарника и карточки instances.
...
- Исправлена проблема v0.1.38, где задача была добавлена в backend scheduler, но не была добавлена в `SchedulerController` и `scheduler.volt`, поэтому не отображалась на странице Scheduler.
